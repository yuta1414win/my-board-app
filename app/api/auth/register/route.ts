  ...
  try {
    ...
    if (existingUser) {
      if (existingUser.emailVerified) {
        ...
        existingUser.emailVerificationExpires = new Date(Date.now() + 24 * 60 * 60 * 1000); // 24時間有効
    ...
    try {
      ...
      return NextResponse.json(
        { 
          error: 'アカウントは作成されましたが、確認メールの送信に失敗しました。サポートにお問い合わせください。',
    ...
    return NextResponse.json({
      message: '登録が完了しました。メールアドレスに確認メール（24時間有効）を送信しました。',
      success: true,
