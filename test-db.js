  ...
  try {
    ...
    if (usersCollection) {
      ...
      if (recentUsers.length > 0) {
        ...
        recentUsers.forEach((user, index) => {
          ...
          console.log(`     認証: ${user.emailVerified ? '済' : '未'}, 作成: ${new Date(user.createdAt).toLocaleString('ja-JP')}`);
        });
        console.log('');
      }
    }
    ...
    if (testUsers.length > 0) {
      ...
      testUsers.forEach((user, index) => {
        console.log(`  ${index + 1}. ${user.name} (${user.email}) - 認証: ${user.emailVerified ? '済' : '未'}`);
      });
    } else {
