...
test.describe('認証機能 スモークテスト', () => {
  ...
  test.describe('基本ページアクセステスト', () => {
    test('ログインページが正常に表示される', async ({ page }) => {
      ...
      // 主要要素の存在確認
      await expect(page.getByRole('heading', { name: /ログイン|Sign In/ })).toBeVisible();
      await expect(page.getByLabel(/メールアドレス|Email/)).toBeVisible();
      await expect(page.getByLabel(/パスワード|Password/)).toBeVisible();
      await expect(page.getByRole('button', { name: /ログイン|Sign In/ })).toBeVisible();
      
      // 登録ページへのリンク確認
      await expect(page.getByRole('link', { name: /新規登録|Register/ })).toBeVisible();
    });
    ...
    test('登録ページが正常に表示される', async ({ page }) => {
      ...
      // 主要要素の存在確認
      await expect(page.getByRole('heading', { name: /登録|Register/ })).toBeVisible();
      await expect(page.getByLabel(/名前|Name/)).toBeVisible();
      await expect(page.getByLabel(/メールアドレス|Email/)).toBeVisible();
      await expect(page.getByLabel(/パスワード|Password/)).toBeVisible();
      await expect(page.getByRole('button', { name: /登録|Register/ })).toBeVisible();
    });

    test('保護されたページへの未認証アクセスがリダイレクトされる', async ({ page }) => {
      // 保護されたページに直接アクセス
  ...
  test.describe('基本認証フロー', () => {
    test('ログイン→ログアウトの基本フローが動作する', async ({ page }) => {
      ...
      await expect(page.getByRole('heading', { name: /掲示板|Board/ })).toBeVisible();
      
      // 5. ユーザー情報表示確認
      await expect(page.getByText(smokeTestUser.name)).toBeVisible();
      
      // 6. ログアウト実行
      const signOutButton = page.getByRole('button', { name: /ログアウト|Sign Out|Logout/ });
      if (await signOutButton.isVisible()) {
        await signOutButton.click();
      } else {
        // メニューからログアウト
        await page.getByRole('button', { name: /メニュー|Menu|Profile/ }).click();
        await page.getByRole('menuitem', { name: /ログアウト|Sign Out/ }).click();
      }
  ...
  test.describe('フォームバリデーション', () => {
    test('登録フォームの必須フィールド検証が動作する', async ({ page }) => {
      ...
      // 空のフォームで送信
      await page.getByRole('button', { name: /登録|Register/ }).click();
  ...
  test.describe('基本パフォーマンス', () => {
    ...
    test('フォーム送信のレスポンス時間が適切である', async ({ page }) => {
      ...
      // エラーメッセージが表示されるまでの時間
      await page.locator('[role="alert"], .error, .alert-error').first().waitFor();
  ...
  test.describe('エラーハンドリング', () => {
    test('存在しないページで404エラーが適切に処理される', async ({ page }) => {
      ...
      if (response) {
        ...
        await expect(page.getByText(/404|ページが見つかりません|Page Not Found/i)).toBeVisible();
      }
    });
