...
test.describe('認証システム エラーケース & セキュリティテスト', () => {
  ...
  test.describe('ネットワークエラーテスト', () => {
    test('APIエンドポイント接続失敗時の適切なエラー表示', async ({ page }) => {
      ...
      await page.getByLabel('パスワード', { exact: true }).fill(testData.validUser.password);
      ...
      await expect(page.getByText('しばらく時間を置いてお試しください')).toBeVisible();
    });
  ...
  test.describe('サーバーエラーテスト', () => {
    ...
    test('429レート制限エラー時の適切な表示', async ({ page }) => {
      ...
      await expect(page.getByText('登録試行回数が上限を超えました')).toBeVisible();
    ...
    test('400バリデーションエラー時の詳細表示', async ({ page }) => {
      await page.route('/api/auth/register', (route) => {
        route.fulfill({
          ...
          body: JSON.stringify({
            ...
            details: [
              { field: 'password', message: 'パスワードは数字、英字、特殊文字を含む必要があります' },
            ],
          }),
        });
      });
      ...
      await expect(page.getByText('パスワードは数字、英字、特殊文字を含む必要があります')).toBeVisible();
    });
  });
  ...
  test.describe('セキュリティテスト', () => {
    ...
    test('SQLインジェクション攻撃への耐性確認', async ({ page }) => {
      ...
      await page.getByLabel('メールアドレス').fill(testData.maliciousInputs.sqlInjectionEmail);
      await page.getByLabel('パスワード', { exact: true }).fill(testData.validUser.password);
      await page.getByLabel('パスワード確認').fill(testData.validUser.password);
    ...
    test('不正な文字入力への適切な対処', async ({ page }) => {
      ...
      // 制御文字や不正な文字を含む入力
      await page.getByLabel('名前').fill(testData.maliciousInputs.invalidCharsName);
    ...
    test('パスワード総当たり攻撃への対策確認', async ({ page }) => {
      ...
      const passwords = ['password1', 'password2', 'password3', 'password4', 'password5', 'password6'];
  ...
  test.describe('入力境界値テスト', () => {
    test('最大長を超える入力の適切な処理', async ({ page }) => {
      ...
      await page.getByLabel('メールアドレス').fill('long-name-test@example.com');
      await page.getByLabel('パスワード', { exact: true }).fill(testData.validUser.password);
      await page.getByLabel('パスワード確認').fill(testData.validUser.password);
    ...
    test('パスワード最大長を超える入力の処理', async ({ page }) => {
      ...
      await page.getByLabel('名前').fill('Test User');
      await page.getByLabel('メールアドレス').fill('long-password-test@example.com');
      await page.getByLabel('パスワード', { exact: true }).fill(testData.maliciousInputs.longPassword);
      await page.getByLabel('パスワード確認').fill(testData.maliciousInputs.longPassword);

      await page.getByRole('button', { name: '登録する' }).click();

      // 適切なバリデーションエラーが表示される
      await expect(page.getByText('100文字以内で入力してください')).toBeVisible();
    });
    ...
    test('Unicode文字の適切な処理', async ({ page }) => {
      ...
      await page.getByLabel('パスワード', { exact: true }).fill(unicodeData.password);
      ...
      const hasError = await page.isVisible('text=エラー');
      const hasSuccess = await page.isVisible('text=成功') || await page.isVisible('text=登録');
      expect(!hasError || hasSuccess).toBeTruthy();
    });
  });
  ...
  test.describe('競合状態テスト', () => {
    ...
    test('同時ログイン試行の適切な処理', async ({ page }) => {
      ...
      await page.getByLabel('メールアドレス').fill('concurrent-test@example.com');
      await page.getByLabel('パスワード').fill('TestPassword123!');
  ...
  test.describe('パフォーマンス境界テスト', () => {
    test('大量データ処理時の適切な応答', async ({ page }) => {
      ...
      await page.getByLabel('メールアドレス').fill('large-data-test@example.com');
      await page.getByLabel('パスワード', { exact: true }).fill(testData.validUser.password);
      await page.getByLabel('パスワード確認').fill(testData.validUser.password);
    ...
    test('低速ネットワーク環境での適切な動作', async ({ page }) => {
      ...
      await page.route('/api/auth/register', async (route) => {
        ...
        route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ success: true, message: '登録が完了しました' }),
        });
      });
  ...
  test.describe('アクセシビリティエラーテスト', () => {
    test('スクリーンリーダー向けエラーメッセージの適切な通知', async ({ page }) => {
      await page.goto('/auth/register');
