...
test.describe('掲示板アプリケーション E2E テスト', () => {
  ...
  test.describe('基本的なナビゲーション', () => {
    test('ホームページから掲示板へ遷移できる', async ({ page }) => {
      // ホームページの要素確認
      await expect(page.locator('h1')).toContainText('オープン掲示板へようこそ');
  ...
  test.describe('投稿編集フロー', () => {
    ...
    test('投稿を編集できる', async ({ page }) => {
      ...
      // 編集ボタンをクリック
      await page.click('button[aria-label*="edit"], [data-testid="EditIcon"]').first();
      
      // 編集ダイアログが表示されることを確認
      await expect(page.locator('text=投稿を編集')).toBeVisible();
      
      // 既存の値が入力されていることを確認
      await expect(page.locator('input[value*="E2E編集テスト投稿"]')).toBeVisible();
      await expect(page.locator('textarea:has-text("これは編集テスト用の投稿です。")')).toBeVisible();
      
      // 内容を編集
      await page.fill('input[value*="E2E編集テスト投稿"]', editedTitle);
      await page.fill('textarea:has-text("これは編集テスト用の投稿です。")', editedContent);
    ...
    test('編集をキャンセルできる', async ({ page }) => {
      ...
      // 内容を一部変更
      await page.fill('input[value*="E2E編集テスト投稿"]', '変更されたタイトル');
  ...
  test.describe('投稿削除フロー', () => {
    ...
    test('投稿を削除できる', async ({ page }) => {
      ...
      // 削除ボタンをクリック
      await page.click('button[aria-label*="delete"], [data-testid="DeleteIcon"]').first();
  ...
  test.describe('レスポンシブデザイン', () => {
    test('モバイル表示でも正常に動作する', async ({ page }) => {
      ...
      await page.fill('textarea[placeholder*="本文"]', 'モバイルでの投稿テストです。');
      await page.click('button:has-text("投稿する")');
  ...
  test.describe('エラーハンドリング', () => {
    test('ネットワークエラー時の動作', async ({ page }) => {
      ...
      await page.fill('textarea[placeholder*="本文"]', 'ネットワークエラーテストです。');
      await page.click('button:has-text("投稿する")');
