...
test.describe('プロフィール機能のテスト', () => {
  ...
  test('3. プロフィール編集機能のテスト', async ({ page }) => {
    ...
    await expect(page.locator('text=更新されたテスト用自己紹介です。')).toBeVisible();
  });
  ...
  test('4. 文字数制限のテスト', async ({ page }) => {
    ...
    // エラーメッセージの確認
    await expect(page.locator('text=一言コメントは50文字以内で入力してください')).toBeVisible();
  ...
  test('5. パスワード変更機能のテスト', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("パスワード変更")')).toBeDisabled();
    ...
    // 保存ボタンが有効になることを確認
    await expect(page.locator('button:has-text("パスワード変更")')).toBeEnabled();
  });
