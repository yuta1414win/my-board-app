...
const tests = {
  ...
  async protectedPagesRedirect() {
    ...
    for (const page of protectedPages) {
      ...
      if (response.status === 200) {
        ...
        throw new Error(`${page} returned unexpected status ${response.status}`);
      }
    }
  ...
  async apiAuthProtection() {
    ...
    for (const api of protectedAPIs) {
      ...
      if (response.status !== 401 && response.status !== 403) {
        throw new Error(`${api} should return 401/403 for unauthenticated requests, got ${response.status}`);
      }
  ...
  async errorPagesCheck() {
    ...
    if (response.status !== 404) {
      throw new Error(`404 page should return status 404, got ${response.status}`);
    }
  ...
  async securityHeadersCheck() {
    ...
    for (const [headerName, expectedValues] of Object.entries(requiredHeaders)) {
...
async function runFunctionalTests() {
  ...
  log(`Test configuration: timeout=${config.timeout}ms, retries=${config.maxRetries}`, 'info');
  ...
  if (testResults.summary.failed > 0) {
    log(`\n⚠️ ${testResults.summary.failed} test(s) failed. Check the details above.`, 'error');
    process.exit(1);
  } else {
