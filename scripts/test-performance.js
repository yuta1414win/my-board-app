...
const performanceTests = {
  ...
  async basicResponseTimeTest() {
    const endpoints = [
      { path: '/', name: 'Homepage', threshold: performanceThresholds.responseTime.homepage },
      { path: '/health', name: 'Health Check', threshold: performanceThresholds.responseTime.api },
      { path: '/auth/signin', name: 'Auth Page', threshold: performanceThresholds.responseTime.homepage },
      { path: '/favicon.ico', name: 'Static Resource', threshold: performanceThresholds.responseTime.static }
    ];
    ...
    results.averageResponseTime = validTests > 0 ? Math.round(totalResponseTime / validTests) : 0;
  ...
  async concurrencyTest() {
    ...
    for (const concurrency of concurrentRequests) {
      ...
      for (let i = 0; i < concurrency; i++) {
        requests.push(
          client.get(testEndpoint).then(response => ({
            status: response.status,
  ...
  async memoryLeakTest() {
    ...
    for (let i = 0; i < iterations; i++) {
      try {
        ...
        if (response.status !== 200) {
          results.warnings.push(`Non-200 response at iteration ${i}: ${response.status}`);
        }
        
      } catch (error) {
        results.warnings.push(`Request failed at iteration ${i}: ${error.message}`);
      }
  ...
  async lighthouseTest() {
    ...
    return new Promise((resolve) => {
      const lighthouseCmd = `npx lighthouse ${config.baseURL} ` +
      ...
      exec(lighthouseCmd, (error, stdout, stderr) => {
        if (error) {
          ...
          results.warnings.push(`Lighthouse execution failed: ${error.message}`);
          resolve(results);
          return;
        }
        ...
        try {
          ...
          if (fs.existsSync('./lighthouse-results.json')) {
            ...
            if (lighthouseData.categories) {
              for (const [category, data] of Object.entries(lighthouseData.categories)) {
          ...
          results.warnings.push(`Failed to parse Lighthouse results: ${parseError.message}`);
        }
  ...
  async resourceUsageTest() {
    ...
    log(`Running resource usage test for ${testDuration / 1000} seconds`, 'info');
...
async function runPerformanceTests() {
  ...
  const performanceScore = Math.round(
    ((testResults.summary.passed + testResults.summary.warnings * 0.7) / testResults.summary.total) * 100
  );
  
  log(`\n🚀 Performance Score: ${performanceScore}%`, performanceScore >= 80 ? 'success' : 'warning');
