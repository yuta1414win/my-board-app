...
async function testRateLimit() {
  return runTest('レート制限テスト', async () => {
    ...
    // 並行してリクエストを送信
    const promises = Array.from({ length: config.rateLimitTests }, async (_, i) => {
      try {
    ...
    for (const payload of xssPayloads) {
      try {
        ...
        if (response.body && response.body.includes(payload)) {
          ...
          log(`XSSペイロード挿入成功: ${payload.substring(0, 50)}...`, 'warning');
        } else {
          blockedPayloads++;
...
async function testAuditLogging() {
  return runTest('監査ログ記録確認テスト', async () => {
    ...
    try {
      ...
      // 監査ログを確認（管理者APIがあれば）
      try {
        const logsResponse = await makeRequest(`${config.baseUrl}/api/admin/audit-logs`, {
          headers: {
  ...
  return runTest('総合セキュリティテスト', async () => {
    ...
    try {
      ...
      if (response.headers['x-powered-by']) {
        vulnerabilities.push(`フレームワーク情報漏洩: ${response.headers['x-powered-by']}`);
      }
      ...
      for (const method of methods) {
        try {
          const methodResponse = await makeRequest(`${config.baseUrl}/`, { method });
