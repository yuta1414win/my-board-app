...
const securityTests = {
    ...
    return new Promise((resolve, reject) => {
      const socket = tls.connect(port, hostname, {
        ...
        if (results.certificate.daysUntilExpiry < 30) {
          results.warnings.push(`Certificate expires in ${results.certificate.daysUntilExpiry} days`);
        }
    ...
    const securityHeaders = {
      'strict-transport-security': {
        ...
        validate: (value) => {
          ...
          if (!value.includes('max-age=')) return { valid: false, message: 'max-age directive missing' };
          
          const maxAge = value.match(/max-age=(\d+)/);
          if (!maxAge || parseInt(maxAge[1]) < 31536000) {
            return { valid: false, message: 'max-age should be at least 1 year (31536000 seconds)' };
          }
      ...
      'content-security-policy': {
        ...
        validate: (value) => {
          ...
          if (value.includes('unsafe-inline') && value.includes('script-src')) {
            return { valid: false, message: 'CSP allows unsafe-inline scripts' };
          }
          return { valid: true };
        }
      },
  ...
  async sqlInjectionTests() {
    ...
    if (results.vulnerableEndpoints.length > 0) {
      throw new Error(`Potential SQL injection vulnerabilities found: ${results.vulnerableEndpoints.length}`);
    }
  ...
  async xssTests() {
    ...
    if (results.vulnerableEndpoints.length > 0) {
      results.warnings.push(`Potential XSS vulnerabilities found: ${results.vulnerableEndpoints.length}`);
    }
  ...
  async rateLimitingTests() {
    ...
    for (const endpoint of endpoints) {
      ...
      if (!endpointResults.rateLimited) {
        results.warnings.push(`${endpoint.path} does not appear to have rate limiting`);
      }
    }
  ...
  async informationDisclosureTests() {
    ...
    if (results.exposedEndpoints.length > 0) {
      results.warnings.push(`Potentially exposed sensitive endpoints: ${results.exposedEndpoints.length}`);
    }
  ...
  async httpSecurityTests() {
    ...
    if (results.poweredBy) {
      results.warnings.push(`X-Powered-By header exposed: ${results.poweredBy}`);
    }
...
async function runSecurityTests() {
  ...
  const securityScore = Math.round(
    ((testResults.summary.passed + testResults.summary.warnings * 0.5) / testResults.summary.total) * 100
  );
  
  log(`\n🛡️ Security Score: ${securityScore}%`, securityScore >= 80 ? 'success' : 'warning');
  
  if (testResults.summary.failed > 0) {
    log(`\n⚠️ Critical security issues found. Immediate attention required!`, 'error');
    process.exit(1);
